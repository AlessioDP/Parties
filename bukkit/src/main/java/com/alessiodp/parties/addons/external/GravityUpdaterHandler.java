...
public class GravityUpdaterHandler {
	/* Constants */
	...
	public static final String DELIMETER = "^v|[\\s_-]v";
	...
	private GravityUpdaterHandler.UpdateResult result = GravityUpdaterHandler.UpdateResult.SUCCESS;
	 ...
	 */
	public GravityUpdaterHandler(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
	 ...
	 */
	public GravityUpdaterHandler(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
	 ...
	 * @param announce True if the program should announce the progress of new updates in console.
	 */
	public GravityUpdaterHandler(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			this.url = new URL(GravityUpdaterHandler.HOST + GravityUpdaterHandler.QUERY + this.id);
	 ...
	 * @see UpdateResult
	 */
	public GravityUpdaterHandler.UpdateResult getResult() {
		this.waitForThread();
		return this.result;
	}
	...
	private void downloadFile() {
		...
		try {
			...
			final byte[] data = new byte[GravityUpdaterHandler.BYTE_SIZE];
			int count;
			if (this.announce) {
				this.plugin.getLogger().info("About to download a new update: " + this.versionName);
			}
			long downloaded = 0;
			while ((count = in.read(data, 0, GravityUpdaterHandler.BYTE_SIZE)) != -1) {
				downloaded += count;
			...
			this.result = GravityUpdaterHandler.UpdateResult.FAIL_DOWNLOAD;
		} finally {
			try {
				if (in != null) {
					in.close();
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (!entry.isDirectory()) {
					...
					int b;
					final byte[] buffer = new byte[GravityUpdaterHandler.BYTE_SIZE];
					final FileOutputStream fos = new FileOutputStream(destinationFilePath);
					final BufferedOutputStream bos = new BufferedOutputStream(fos, GravityUpdaterHandler.BYTE_SIZE);
					while ((b = bis.read(buffer, 0, GravityUpdaterHandler.BYTE_SIZE)) != -1) {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(DELIMETER).length >= 2) {
				...
				if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
					...
					this.result = GravityUpdaterHandler.UpdateResult.NO_UPDATE;
					return false;
				}
			} else {
				...
				this.result = GravityUpdaterHandler.UpdateResult.FAIL_NOVERSION;
				return false;
			}
		}
		return true;
	...
	private boolean hasTag(String version) {
		for (final String string : GravityUpdaterHandler.NO_UPDATE_TAG) {
	...
	private boolean read() {
		try {
			...
			conn.addRequestProperty("User-Agent", GravityUpdaterHandler.USER_AGENT);
			...
			this.versionName = (String) latestUpdate.get(GravityUpdaterHandler.TITLE_VALUE);
			this.versionLink = (String) latestUpdate.get(GravityUpdaterHandler.LINK_VALUE);
			this.versionType = (String) latestUpdate.get(GravityUpdaterHandler.TYPE_VALUE);
			this.versionGameVersion = (String) latestUpdate.get(GravityUpdaterHandler.VERSION_VALUE);
	...
	public interface UpdateCallback {
		 ...
		 */
		void onFinish(GravityUpdaterHandler updater);
	}
