...
public class CommandChat implements CommandInterface {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (tp.chatParty()) {
			...
			LogHandler.log(LogLevel.MEDIUM, p.getName() + "[" + p.getUniqueId() + "] toggled party chat in true", true);
		} else {
			tp.sendMessage(Messages.chat_disabled);
			LogHandler.log(LogLevel.MEDIUM, p.getName() + "[" + p.getUniqueId() + "] toggled party chat in false", true);
		}
		return true;
	}
}
