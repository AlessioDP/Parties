	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if(args.length == 1){
			...
			tp.sendMessage(Messages.ignore_header
					.replace("%number%", ""+tp.getIgnoredParties().size()));
		...
		if(tp.getIgnoredParties().contains(ignoredParty)){
			...
			LogHandler.log(2, p.getName() + "[" + p.getUniqueId() + "] stopped ignore party " + ignoredParty);
		} else {
			tp.addIgnoredParty(ignoredParty);
			tp.sendMessage(Messages.ignore_ignored.replace("%party%", ignoredParty));
			LogHandler.log(2, p.getName() + "[" + p.getUniqueId() + "] now ignore party " + ignoredParty);
		}
		return true;
	}
}
