...
public class CommandPassword implements CommandInterface{
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (args[1].equalsIgnoreCase(Variables.tag_custom_removeword)) {
			...
			party.sendSpyMessage(p, Messages.password_removed);
			
			LogHandler.log(3, p.getName() + "[" + p.getUniqueId() + "] removed password to the party " + party.getName());
			return true;
		}
		...
		party.sendSpyMessage(p, Messages.password_changed);
		
		LogHandler.log(3, p.getName() + "[" + p.getUniqueId() + "] setted new password to the party " + party.getName());
		return true;
	}
