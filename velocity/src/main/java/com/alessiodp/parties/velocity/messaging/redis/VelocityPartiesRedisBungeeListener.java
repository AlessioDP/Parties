...
public class VelocityPartiesRedisBungeeListener extends VelocityRedisBungeeListener {
	private final CommonListener commonListener;
	
	public VelocityPartiesRedisBungeeListener(ADPPlugin plugin) {
	...
	protected void onMessageEvent(byte[] bytes) {
		...
		if (packet != null) {
			if (packet.getSource().equals(VelocityRedisBungeeHandler.getProxyId()))
				return;
