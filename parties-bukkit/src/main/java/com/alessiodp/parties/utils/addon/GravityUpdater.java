...
public class GravityUpdater {
	...
	private GravityUpdater.UpdateResult result = GravityUpdater.UpdateResult.SUCCESS;
	 ...
	 */
	public GravityUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
	 ...
	 * @param announce True if the program should announce the progress of new updates in console.
	 */
	public GravityUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			this.url = new URL(GravityUpdater.HOST + GravityUpdater.QUERY + this.id);
	...
	private void downloadFile() {
		...
		try {
			...
			final byte[] data = new byte[GravityUpdater.BYTE_SIZE];
			int count;
			if (this.announce) {
				this.plugin.getLogger().info("About to download a new update: " + this.versionName);
			}
			long downloaded = 0;
			while ((count = in.read(data, 0, GravityUpdater.BYTE_SIZE)) != -1) {
				downloaded += count;
			...
			this.result = GravityUpdater.UpdateResult.FAIL_DOWNLOAD;
		} finally {
			try {
				if (in != null) {
					in.close();
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (!entry.isDirectory()) {
					...
					int b;
					final byte[] buffer = new byte[GravityUpdater.BYTE_SIZE];
					final FileOutputStream fos = new FileOutputStream(destinationFilePath);
					final BufferedOutputStream bos = new BufferedOutputStream(fos, GravityUpdater.BYTE_SIZE);
					while ((b = bis.read(buffer, 0, GravityUpdater.BYTE_SIZE)) != -1) {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(DELIMETER).length >= 2) {
				...
				if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
					...
					this.result = GravityUpdater.UpdateResult.NO_UPDATE;
					return false;
				}
			} else {
				...
				this.result = GravityUpdater.UpdateResult.FAIL_NOVERSION;
				return false;
			}
		}
		return true;
	...
	private boolean hasTag(String version) {
		for (final String string : GravityUpdater.NO_UPDATE_TAG) {
	...
	private boolean read() {
		try {
			...
			conn.addRequestProperty("User-Agent", GravityUpdater.USER_AGENT);
			...
			this.versionName = (String) latestUpdate.get(GravityUpdater.TITLE_VALUE);
			this.versionLink = (String) latestUpdate.get(GravityUpdater.LINK_VALUE);
			this.versionType = (String) latestUpdate.get(GravityUpdater.TYPE_VALUE);
			this.versionGameVersion = (String) latestUpdate.get(GravityUpdater.VERSION_VALUE);
