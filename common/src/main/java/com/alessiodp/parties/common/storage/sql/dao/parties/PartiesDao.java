...
public interface PartiesDao {
	...
	String QUERY_GET_LIST_BY_NAME = "SELECT * FROM `<prefix>parties` <if(blacklist)>WHERE name NOT IN (<blacklist>) AND id NOT IN (<blacklist>)<endif> ORDER BY `name` ASC,`id` ASC LIMIT :limit OFFSET :offset";
