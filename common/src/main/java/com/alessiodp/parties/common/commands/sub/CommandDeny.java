	...
	public boolean preRequisites(@NotNull CommandData commandData) {
		...
		if (ret && ((PartiesCommandData) commandData).getPartyPlayer().getPartyId() != null) {
			if ((ConfigParties.ADDITIONAL_ASK_ENABLE
					&& getPlugin().getRankManager().checkPlayerRankAlerter(((PartiesCommandData) commandData).getPartyPlayer(), RankPermission.ASK_DENY)
			)
					|| (ConfigParties.ADDITIONAL_TELEPORT_ENABLE
					&& ConfigParties.ADDITIONAL_TELEPORT_ACCEPT_REQUEST_ENABLE
					&& getPlugin().getRankManager().checkPlayerRankAlerter(((PartiesCommandData) commandData).getPartyPlayer(), RankPermission.TELEPORT_DENY))) {
